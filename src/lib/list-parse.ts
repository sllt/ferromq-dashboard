...
export type HeaderMap = Record<string, unknown> & { get?: (key: string) => unknown }

function headerValue(headers: HeaderMap | undefined, name: string): string | undefined {
  if (!headers) return undefined
  const lower = name.toLowerCase()
  const raw = headers[name] ?? headers[lower] ?? (typeof headers.get === 'function' ? headers.get(name) : undefined)
  if (Array.isArray(raw)) return raw[0] == null ? undefined : String(raw[0])
  if (raw == null) return undefined
  return String(raw)
}

function parseIntHeader(headers: HeaderMap | undefined, name: string): number | undefined {
...
function parseBoolHeader(headers: HeaderMap | undefined, name: string): boolean | undefined {
